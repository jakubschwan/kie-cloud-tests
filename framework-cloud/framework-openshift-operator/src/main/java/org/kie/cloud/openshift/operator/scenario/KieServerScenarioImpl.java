...
public class KieServerScenarioImpl extends OpenShiftOperatorScenario<KieServerScenario> implements KieServerScenario {
    ...
    protected void deployCustomResource() {
        if (request.isDeploySso()) {
            ...
            URL ssoSecureUrl = ssoDeployment.getSecureUrl().orElseThrow(() -> new RuntimeException(
                    "RH SSO secure URL not found."));
        ...
        new SupplierWaiter<KieApp>(() -> getKieAppClient().withName(OpenShiftConstants.getKieApplicationName()).get(),
                kieApp -> kieApp.getStatus() != null).reason("Waiting for reconciliation to initialize all fields.")
        ...
        try {
            new SimpleWaiter(() -> kieServerDeployment.isReady()).reason(
                    "Waiting for Kie server service to be created.").timeout(TimeUnit.MINUTES, 1).waitFor();
